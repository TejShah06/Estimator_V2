...
from app.services.pdf_service import generate_ai_report_pdf
...
async def analyze(
    ...
    usage = check_usage_limit(db, current_user.id, "ai_analysis")
    ...
    if not usage["allowed"]:
        raise HTTPException(
            ...
            detail={
                "error":            "limit_reached",
                "message":          f"You have used {usage['current']}/{usage['limit']} AI analyses this month.",
    ...
    try:
        ...
        # Project name logic
        final_project_name = (
            project_name if project_name and project_name.strip() else None
        )
        ...
        logger.info(
            ...
            f"('{final_project_name}') for user {current_user.id}"
        )
        ...
        new_count = increment_usage(db, current_user.id, "ai_analysis")
        logger.info(
            f"AI usage incremented for user {current_user.id}: "
...
async def analyze_custom(
    ...
    try:
        ...
        logger.info(
            f"Analyzing with custom rates: {file.filename} "
...
@router.get("/report/{project_id}/download-pdf")
async def download_ai_report_pdf(
    ...
    allowed = check_pdf_permission(db, current_user.id, "ai")
    ...
    if not allowed:
        raise HTTPException(
            ...
            detail={
                ...
                "message":          "Upgrade to Advanced or Extreme plan to download AI reports as PDF.",
                "upgrade_required": True,
            }
        )
    ...
    # ── Generate PDF ────────────────────────────────────────────────────────
    pdf_buffer = generate_ai_report_pdf(report)
    filename   = f"AI_Report_{project.project_name or project_id}.pdf"
