...
async def upload_and_generate_3d(
    ...
    # ── STEP 1: Check 3D usage limit BEFORE processing ──────────────────
    usage = check_usage_limit(db, current_user.id, "three_d")
    ...
    if not usage["allowed"]:
        raise HTTPException(
            ...
            detail={
                "error":        "limit_reached",
                "message":      f"You have used {usage['current']}/{usage['limit']} 3D renders this month.",
    ...
    try:
        logger.info(
            f"User {current_user.id} uploading floorplan: {file.filename}"
        )
        ...
        # ── STEP 3: Generate 3D Model in Memory ────────────────────────────
        ...
        # ── STEP 4: Increment usage ONLY after success ───────────────────
        new_count = increment_usage(db, current_user.id, "three_d")
        logger.info(
            f"3D usage incremented for user {current_user.id}: "
            f"{new_count}/{usage['limit']}"
        )

        # ── STEP 5: Return GLB Binary ───────────────────────────────────────
        return StreamingResponse(
            io.BytesIO(glb_bytes),
            media_type="application/octet-stream",
            headers={
                "Content-Disposition":
                    f'attachment; filename="floorplan_{project_id}.glb"',
                ...
                "X-Usage-Current":    str(new_count),
                "X-Usage-Limit":      str(usage["limit"]),
                "Access-Control-Expose-Headers": (
                    "X-Project-ID,X-Generation-Time,X-File-Size,"
                    "X-Wall-Count,X-Door-Count,X-Window-Count,"
                    "X-Usage-Current,X-Usage-Limit"
                )
            }
        )
        ...
        logger.error(
            f"Upload and Generate Error: {str(e)}", exc_info=True
        )
        raise HTTPException(
            status_code=500,
    ...
    try:
        ...
        return StreamingResponse(
            ...
            headers={
                ...
                "X-Usage-Limit":     str(usage["limit"]),
                "Access-Control-Expose-Headers": (
                    "X-Project-ID,X-Generation-Time,"
                    "X-File-Size,X-Usage-Current,X-Usage-Limit"
                )
            }
        )
...
# ── NEW: Check GLB download permission ─────────────────────────────────────
@router.get("/check-download-permission")
def check_glb_download_permission(
    db:           Session = Depends(get_db),
    current_user: User    = Depends(get_current_user),
):
    """
    Check if current user can download GLB files.
    Basic plan →   cannot download GLB
    Advanced / Extreme → can download GLB
    """
    allowed = check_pdf_permission(db, current_user.id, "3d")

    return {
        "allowed": allowed,
        "message": (
            "GLB download allowed"
            if allowed
            else "Upgrade to Advanced or Extreme plan to download GLB files"
        )
    }