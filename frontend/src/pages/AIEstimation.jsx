...
import {
  Upload, Zap, Calculator, FileText, DollarSign,
  CheckCircle2, AlertCircle, Loader2, Download,
  Home, Square, Ruler, PaintBucket, DoorOpen,
  Frame, Lightbulb, Droplet, TrendingUp, Eye,
  Settings, Type
} from 'lucide-react';
...
const AIEstimation = () => {
  ...
  const rateFields = [
    { key: 'flooring_cost_per_sqft',      label: 'Flooring Cost (per sqft)',      icon: Square,      prefix: '₹', description: 'Cost per square foot for flooring materials and installation' },
    { key: 'wall_paint_cost_per_sqft',    label: 'Wall Paint Cost (per sqft)',    icon: PaintBucket, prefix: '₹', description: 'Cost per square foot for wall painting' },
    { key: 'ceiling_paint_cost_per_sqft', label: 'Ceiling Paint Cost (per sqft)', icon: PaintBucket, prefix: '₹', description: 'Cost per square foot for ceiling painting' },
    { key: 'door_unit_cost',              label: 'Door Unit Cost',                icon: DoorOpen,    prefix: '₹', description: 'Cost per door including frame and installation' },
    { key: 'window_unit_cost',            label: 'Window Unit Cost',              icon: Frame,       prefix: '₹', description: 'Cost per window including frame and installation' },
    { key: 'wall_height_ft',              label: 'Wall Height (feet)',            icon: Ruler,       suffix: 'ft', description: 'Average wall height for calculations' },
    { key: 'electrical_per_room',         label: 'Electrical Work (per room)',    icon: Lightbulb,   prefix: '₹', description: 'Average electrical work cost per room' },
    { key: 'plumbing_per_wet_room',       label: 'Plumbing (per wet room)',       icon: Droplet,     prefix: '₹', description: 'Plumbing cost for bathrooms and kitchens' },
  ];
  ...
  const proceedToRates = () => {
    if (!file) { setError('Please select a floor plan image'); return; }
    setStep(2);
  };
                  ...
                  <div className="mb-8 p-6 bg-slate-800/50 rounded-xl">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <h3 className="text-lg font-semibold text-white mb-2">Use Custom Rates</h3>
            ...
            {step === 3 && (
              ...
              >
                <div className="bg-slate-900/50 backdrop-blur-xl border border-white/10 rounded-2xl p-12 text-center">
                  ...
                  <h3 className="text-2xl font-bold text-white mb-2">Analyzing Floor Plan</h3>
                  {projectName && (
                    <p className="text-cyan-400 font-medium mb-2">Project: {projectName}</p>
                  )}
